			...
			if (current.getLabel().equals(goal)) {
				...
				for (Edge ch : childr) {
					...
					if (!choDuyet.contains(node) && !extend.contains(node)) {
						...
						node.setPathCost(current.getPathCost()+ch.getWeight());
					}else if(choDuyet.contains(node) && node.getPathCost()>current.getPathCost()){
						node.setPathCost(current.getPathCost());
						node.setParent(current);
					}
				}
			}
