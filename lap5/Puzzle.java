...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for(int i=0;i<MAX_ROW;i++) { 
			for(int j=0;j<MAX_COL;j++) {
				int tileCurrrent = currentState.getTile(i, j);
				int tileGoal = goalState.getTile(i, j);
				if(tileCurrrent!=0&&tileCurrrent!=tileGoal) {
					output++;
				}
	...
	public int computeH2(Node currentState) {
		...
		for(int tile =1;tile<MAX_COL*MAX_ROW;tile++) {
			int []current_location=currentState.getLocation(tile);
			int[]goal_location=goalState.getLocation(tile);
			result+=PuzzleUtils.manhattanDistance(current_location, goal_location);
		}
		return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			if (row >= 0) {// Tile stands inside the map
				int tmp = currentState.getTile(row, col);// tile da duoc di chuyen
			...
			int col=whiteTile[1];
			if(row<MAX_ROW) {
				int tile_moved=currentState.getTile(row, col);
				result.updateTile(row, col, 0);
				result.updateTile(whiteTile[0], whiteTile[1], tile_moved);
				result.setH(computeH2(result));
				return result;
			}
